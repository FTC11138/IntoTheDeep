...
public class Robot extends MecanumDrive {
    ...
    public void initialize(HardwareMap hardwareMap, Telemetry telemetry_) {
        ...
        this.depositSubsystem = new DepositSubsystem(hardwareMap, config.lift, config.lift2, config.bucket);
        this.intakeSubsystem = new IntakeSubsystem(hardwareMap, config.extension, config.arm1, config.arm2, config.intake, config.dist, config.leds);
